...
---
每次在本地创建、修改任何文件（包括 `~/.claude/projects/-Users-n0cytra/memory/` 下的 memory 文件），都要同步备份一份到 `/Users/n0cytra/N0ctyra/MacClaude/` 对应位置，并 `git commit/push` 推到远程仓库。
...
3. 操作完成后执行 `cd /Users/n0cytra/N0ctyra && git add . && git commit -m "..." && git push`。
